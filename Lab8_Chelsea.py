...
def remove_duplicates_set(s):
    ...
    Returns: 
    -(string): String with duplicate characters removed.
    """
    seen_set = set()
    res = ''
    for c in s:
...
def gem_counting(stones, gems):
    """
    This function counts how many stones are actually gems using sets.
    ...
    Parameters:
    -stones (string): String containing stone types collected.
    -gems (stirng): String containing gem types.
    
    Returns:
    -(int): Number of stones that match gem types.
    """
    stone_set = set(stones)
    gem_set = set(gems)
    return len(stone_set & gem_set)
...
def students_id(ids):
    """
    This function counts the number of unique student IDs who use the shuttle service.
    ...
    Parameters:
    -ids (list): List of student IDs as strings.
    
    Returns:
    -(int): Number of different students using shuttle service.
    """
    unique_ids = set(ids)
...
def students_id_occurrences(ids):
    """
    This function counts occurrences of each unique student ID using a dictionary.
    
    E.g., 
    >>> students_id_occurrences(['002', '003', '001', '004', '012'])
    returns {'002': 1, '003': 1, '001': 1, '004':1, '012': 1}
    >>> students_id_occrrences(['002', '003', '001', '012', '003', '001'])
    returns {'002': 1, '003': 2, '001': 2, '012': 1}
    ...
    Returns:
    -(dict): Dictionary mapping each student ID to their occrrence count.
    """
    id_dict = {}
...
def word_frequency(paragraph):
    """
    This function counts the frequency of words in a paragraph using dictionary.
    ...
    Parameters:
    -paragraph (string): Input text paragraph.
